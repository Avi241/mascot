...
bool DroneObjectROS::move_x(float force) {

  twist_msg.linear.x = force;
  pubCmd.publish(twist_msg);
  ROS_INFO("Force_x...");
  return true;
}

bool DroneObjectROS::move_y(float force) {


  twist_msg.linear.y = force;
  pubCmd.publish(twist_msg);
  ROS_INFO("Force_y...");
  return true;
}

bool DroneObjectROS::stop_force() {
